...
O Programa do evento permite criar e editar pontos com tipo, início, fim, local, responsável e notas, eliminar registos e reordená-los por arrastar, com persistência atómica da sequência. A reordenação é validada no servidor pela RPC `reorder_event_program_v1`, que confirma permissões, pertença ao evento e integridade da lista antes de normalizar a sequência. A gestão de Incidentes permite registar data/hora, título, descrição, local, gravidade, estado, responsável e resolução, mantendo `resolved_at` coerente ao resolver ou reabrir um incidente e disponibilizando edição e eliminação aos gestores de operações.
