...
class _AssetsScreenState extends State<AssetsScreen> {
  ...
  Future<void> _addLookup({required bool location}) async {
    final controller = TextEditingController();
    final value = await showDialog<String>(
      context: context,
      builder: (context) => AlertDialog(
        title: Text(location ? 'Nova localização' : 'Nova categoria'),
        content: TextField(
          controller: controller,
          autofocus: true,
          decoration: InputDecoration(
            labelText: location ? 'Nome da localização' : 'Nome da categoria',
    ...
    try {
      if (location) {
        await _repository.addLocation(value);
      } else {
  ...
  Future<void> _edit(_AssetsData data, [Map<String, dynamic>? asset]) async {
    try {
      ...
      if (result.image != null) {
        await _repository.uploadPrimaryImage(assetId: saved['id'].toString(), file: result.image!);
      }
      if (mounted) setState(_reload);
    } catch (error) {
      _error(error);
      ...
      builder: (context, snapshot) {
        ...
        final filtered = data.assets.where((asset) {
          ...
          return (query.isEmpty || haystack.contains(query)) &&
              (_category == null || asset['category']?.toString() == _category) &&
              (_condition == null || asset['condition']?.toString() == _condition) &&
              (_locationId == null || asset['location_id']?.toString() == _locationId) &&
              (_responsibleId == null || asset['responsible_member_id']?.toString() == _responsibleId);
        }).toList();

        final active = data.assets.where((a) => a['condition'] != 'retired');
          ...
          body: RefreshIndicator(
            ...
            child: ListView(
              ...
              children: [
                ...
                Wrap(spacing: 10, runSpacing: 10, children: [
                  _Filter<String>(label: 'Categoria', value: _category, items: data.categories.map((e) => e['name'].toString()).toList(), text: (v) => v, onChanged: (v) => setState(() => _category = v)),
                  _Filter<String>(label: 'Estado', value: _condition, items: _conditions.keys.toList(), text: (v) => _conditions[v]!, onChanged: (v) => setState(() => _condition = v)),
                  _Filter<String>(label: 'Localização', value: _locationId, items: data.locations.map((e) => e['id'].toString()).toList(), text: (v) => data.locations.firstWhere((e) => e['id'].toString() == v)['name'].toString(), onChanged: (v) => setState(() => _locationId = v)),
                  _Filter<String>(label: 'Responsável', value: _responsibleId, items: data.members.map((e) => e['id'].toString()).toList(), text: (v) => _memberName(data.members.firstWhere((e) => e['id'].toString() == v)), onChanged: (v) => setState(() => _responsibleId = v)),
                  if (_canManage) OutlinedButton.icon(onPressed: () => _addLookup(location: false), icon: const Icon(Icons.category_outlined), label: const Text('Nova categoria')),
                  if (_canManage) OutlinedButton.icon(onPressed: () => _addLookup(location: true), icon: const Icon(Icons.add_location_alt_outlined), label: const Text('Nova localização')),
                  if (_category != null || _condition != null || _locationId != null || _responsibleId != null)
                    OutlinedButton.icon(
                      onPressed: () => setState(() { _category = null; _condition = null; _locationId = null; _responsibleId = null; }),
                ...
                const SizedBox(height: 16),
                if (filtered.isEmpty)
                  const Card(child: ListTile(leading: Icon(Icons.home_repair_service_outlined), title: Text('Nenhum bem encontrado.'), subtitle: Text('Cria o primeiro bem ou altera os filtros.')))
                else
                  for (final asset in filtered)
                    _AssetCard(repository: _repository, asset: asset, onTap: () => _detail(data, asset)),
              ],
            ),
          ),
          floatingActionButton: _canManage
              ? FloatingActionButton.extended(onPressed: () => _edit(data), icon: const Icon(Icons.add), label: const Text('Novo bem'))
              : null,
        );
      },
    );
          ...
          child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            Expanded(
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Row(children: [
                  Expanded(child: Text(asset['name']?.toString() ?? 'Bem', style: Theme.of(context).textTheme.titleMedium?.copyWith(fontWeight: FontWeight.w800))),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: ListView(
        ...
        children: [
          if (image != null) ClipRRect(borderRadius: BorderRadius.circular(18), child: Image.network(image, height: 220, fit: BoxFit.cover)),
          ...
          _InfoCard(title: 'Aquisição e valor', rows: {
            'Data aquisição': _formatDate(asset['acquisition_date']), 'Fornecedor': asset['supplier'],
            'Valor aquisição': _money(asset['acquisition_value']), 'Valor atual': _money(asset['current_value']),
          ...
          if (asset['requires_inspection'] == true)
            _InfoCard(title: 'Inspeção periódica', rows: {
              ...
              'Última inspeção': _formatDate(asset['last_inspection_at']), 'Próxima inspeção': _formatDate(asset['next_inspection_at']),
            }),
          ...
          FutureBuilder<List<Map<String, dynamic>>>(
            ...
            builder: (context, snapshot) {
              ...
              return Column(children: snapshot.data!.map((event) {
                ...
                return Card(child: ListTile(
                  ...
                  subtitle: Text([event['description']?.toString(), if (date != null) DateFormat('dd/MM/yyyy HH:mm').format(date)].whereType<String>().where((e) => e.isNotEmpty).join(' · ')),
                ));
              }).toList());
            },
          ),
...
class _AssetEditor extends StatefulWidget {
  ...
  final List<Map<String, dynamic>> categories, locations, members;
...
class _AssetEditorState extends State<_AssetEditor> {
  late final TextEditingController _name, _description, _brand, _model, _serial, _supplier, _acquisitionValue, _currentValue, _notes, _specific1, _specific2;
  String _category = 'Outros';
  String _condition = 'good';
  String? _locationId, _responsibleId;
  DateTime? _acquisitionDate, _warrantyUntil, _lastInspection, _nextInspection;
  ...
  Future<DateTime?> _date(DateTime? current) => showDatePicker(context: context, initialDate: current ?? DateTime.now(), firstDate: DateTime(2000), lastDate: DateTime(2100));
  ...
  Widget _section(String title, IconData icon, List<Widget> children) => Card(
    ...
    child: Padding(padding: const EdgeInsets.all(14), child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
      ...
      const SizedBox(height: 14), ...children.map((e) => Padding(padding: const EdgeInsets.only(bottom: 12), child: e)),
    ])),
  );
    ...
    return Scaffold(
      ...
      body: ListView(padding: const EdgeInsets.all(16), children: [
        _section('Identificação', Icons.badge_outlined, [
          Wrap(spacing: 16, runSpacing: 4, children: [Text('Código: ${widget.codes['asset_number']}'), Text('QR: ${widget.codes['qr_code']}')]),
          TextField(controller: _name, decoration: _dec('Nome *')),
          DropdownButtonFormField<String>(initialValue: _category, isExpanded: true, decoration: _dec('Categoria'), items: widget.categories.map((c) => DropdownMenuItem(value: c['name'].toString(), child: Text(c['name'].toString()))).toList(), onChanged: (v) { if (v != null) setState(() { _category=v; _specific1.clear(); _specific2.clear(); }); }),
          DropdownButtonFormField<String>(initialValue: _condition, isExpanded: true, decoration: _dec('Estado'), items: _conditions.entries.map((e) => DropdownMenuItem(value: e.key, child: Text(e.value))).toList(), onChanged: (v) => setState(() => _condition=v ?? _condition)),
          TextField(controller: _description, maxLines: 3, decoration: _dec('Descrição')),
        ]),
        _section('Fotografia', Icons.photo_camera_outlined, [
          if (_image != null)
            ListTile(leading: const CircleAvatar(child: Icon(Icons.image_outlined)), title: const Text('Nova fotografia selecionada'), subtitle: Text(_image!.name))
        ...
        _section('Localização e responsável', Icons.place_outlined, [
          DropdownButtonFormField<String?>(initialValue: _locationId, isExpanded: true, decoration: _dec('Localização'), items: [const DropdownMenuItem<String?>(value: null, child: Text('Sem localização')), ...widget.locations.map((l) => DropdownMenuItem<String?>(value: l['id'].toString(), child: Text(l['name'].toString())))], onChanged: (v) => setState(() => _locationId=v)),
          DropdownButtonFormField<String?>(initialValue: _responsibleId, isExpanded: true, decoration: _dec('Responsável'), items: [const DropdownMenuItem<String?>(value: null, child: Text('Sem responsável')), ...widget.members.map((m) => DropdownMenuItem<String?>(value: m['id'].toString(), child: Text(_memberName(m))))], onChanged: (v) => setState(() => _responsibleId=v)),
        ]),
        ...
        _section('Aquisição e valor', Icons.euro_outlined, [
          _DateTile(label: 'Data de aquisição', value: _acquisitionDate, onTap: () async { final v=await _date(_acquisitionDate); if (v!=null && mounted) setState(() => _acquisitionDate=v); }),
          ...
          _DateTile(label: 'Garantia até', value: _warrantyUntil, onTap: () async { final v=await _date(_warrantyUntil); if (v!=null && mounted) setState(() => _warrantyUntil=v); }),
        ]),
        _section('Inspeção periódica', Icons.fact_check_outlined, [
          ...
          if (_requiresInspection) ...[
            DropdownButtonFormField<int>(initialValue: _inspectionMonths, decoration: _dec('Periodicidade'), items: const [3,6,12,24].map((m) => DropdownMenuItem(value:m, child:Text('$m meses'))).toList(), onChanged: (v) => setState(() => _inspectionMonths=v)),
            _DateTile(label:'Última inspeção', value:_lastInspection, onTap:() async { final v=await _date(_lastInspection); if(v!=null && mounted) setState(() => _lastInspection=v); }),
            _DateTile(label:'Próxima inspeção', value:_nextInspection, onTap:() async { final v=await _date(_nextInspection); if(v!=null && mounted) setState(() => _nextInspection=v); }),
          ],
        ]),
        ...
        FilledButton.icon(
          onPressed: () {
            ...
            Navigator.pop(context,_AssetInput(
              assetNumber:widget.codes['asset_number']!,qrCode:widget.codes['qr_code']!,name:_name.text,category:_category,description:_description.text,
              brand:_brand.text,model:_model.text,serialNumber:_serial.text,condition:_condition,locationId:_locationId,responsibleMemberId:_responsibleId,
              acquisitionDate:_acquisitionDate,acquisitionValue:_parse(_acquisitionValue.text),currentValue:_parse(_currentValue.text),supplier:_supplier.text,
              warrantyUntil:_warrantyUntil,requiresInspection:_requiresInspection,inspectionIntervalMonths:_inspectionMonths,lastInspectionAt:_lastInspection,
              nextInspectionAt:_nextInspection,customAttributes:custom,notes:_notes.text,image:_image,
            ));
          },
          icon: const Icon(Icons.save_outlined), label: const Text('Guardar bem'),
        ),
...
class _DateTile extends StatelessWidget {
  ...
  final VoidCallback onTap;
  @override
  Widget build(BuildContext context) => ListTile(contentPadding:EdgeInsets.zero,title:Text(label),subtitle:Text(_dateText(value)),trailing:const Icon(Icons.calendar_month_outlined),onTap:onTap);
}
...
class _Filter<T> extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) => SizedBox(width:205,child:DropdownButtonFormField<T?>(initialValue:value,isExpanded:true,decoration:InputDecoration(labelText:label,border:const OutlineInputBorder()),items:[DropdownMenuItem<T?>(value:null,child:const Text('Todos')),...items.map((v)=>DropdownMenuItem<T?>(value:v,child:Text(text(v),overflow:TextOverflow.ellipsis)))],onChanged:onChanged));
}
...
class _InfoCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Card(child:Padding(padding:const EdgeInsets.all(14),child:Column(crossAxisAlignment:CrossAxisAlignment.stretch,children:[
      Text(title,style:Theme.of(context).textTheme.titleMedium?.copyWith(fontWeight:FontWeight.w800)),const SizedBox(height:8),
...
const _conditions=<String,String>{'excellent':'Excelente','good':'Bom','regular':'Regular','maintenance':'Necessita manutenção','damaged':'Avariado','retired':'Abatido'};
IconData _conditionIcon(String condition)=>switch(condition){'excellent'=>Icons.verified_outlined,'good'=>Icons.check_circle_outline,'regular'=>Icons.info_outline,'maintenance'=>Icons.build_outlined,'damaged'=>Icons.error_outline,'retired'=>Icons.archive_outlined,_=>Icons.circle_outlined};
(String?,String?,String?,String?) _attributeLabels(String category){final c=category.toLowerCase();if(c.contains('gerador'))return('Potência','Combustível','potencia','combustivel');if(c.contains('som'))return('Potência','N.º de colunas','potencia','numero_colunas');if(c.contains('tenda'))return('Dimensões','Material','dimensoes','material');if(c.contains('ilumina'))return('Potência','Tipo','potencia','tipo');return(null,null,null,null);}
String _memberName(Map<String,dynamic> member){final nick=member['nickname']?.toString().trim()??'';final full=member['full_name']?.toString().trim()??'Membro';return nick.isEmpty?full:'$full ($nick)';}
